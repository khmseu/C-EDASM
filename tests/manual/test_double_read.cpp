...
void test_single_vs_double_read() {
    ...
    std::cout << "   - Reading back from $D000: $" << std::hex << std::uppercase << std::setw(2)
              << std::setfill('0') << (int)value << std::endl;

    if (value == 0xFF) {
        std::cout << "   ✓ CORRECT: Write was NOT enabled (still contains init value $FF)"
                  << std::endl;
    } else if (value == 0xAA) {
        std::cout << "   ✗ WRONG: Write was enabled (RAM was written)" << std::endl;
    } else {
        std::cout << "   ? UNEXPECTED: Got unexpected value $" << std::hex << (int)value
                  << std::endl;
    }
    std::cout << std::endl;
    ...
    if (value == 0xBB) {
        ...
        std::cout << "   ✗ WRONG: Write was NOT enabled (got $" << std::hex << (int)value << ")"
                  << std::endl;
    }
    std::cout << std::endl;
    ...
    std::cout << "   - Value: $" << std::hex << std::uppercase << std::setw(2) << std::setfill('0')
              << (int)value << std::endl;

    if (value == 0xFF) {
        std::cout << "   ✓ CORRECT: Reading ROM (not the written RAM)" << std::endl;
    } else {
        std::cout << "   ✗ WRONG: Reading RAM instead of ROM (got $" << std::hex << (int)value
                  << ")" << std::endl;
    }
