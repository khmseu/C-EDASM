...
void test_lookup_correctness() {
    ...
    const uint8_t invalid_calls[] = {
        0x00, 0x01, 0x42, 0x7F, 0xFF, 0xBF, 0xD4, 0xD5
    };
    
    std::cout << "Testing invalid call numbers..." << std::endl;
    for (uint8_t call_num : invalid_calls) {
        std::cout << "  Testing call 0x" << std::hex << std::uppercase 
                  << std::setw(2) << std::setfill('0') 
                  << static_cast<int>(call_num) << "..." << std::flush;
        const MLICallDescriptor *desc = MLIHandler::get_call_descriptor(call_num);
        if (desc != nullptr) {
            std::cerr << std::endl << "ERROR for call 0x" << std::hex << std::uppercase 
                      << std::setw(2) << std::setfill('0') 
                      << static_cast<int>(call_num) << ": got descriptor!" << std::endl;
            std::cerr << "  Descriptor address: " << static_cast<const void*>(desc) << std::endl;
            std::cerr << "  Descriptor call_number: 0x" << std::hex << std::uppercase
                      << std::setw(2) << std::setfill('0')
                      << static_cast<int>(desc->call_number) << std::dec << std::endl;
...
void benchmark_lookup() {
    ...
    const uint8_t test_calls[] = {
        0xC8, 0xCA, 0xCC, 0xFF, 0x82, 0xC4, 0x00, 0xC6,
    ...
    double avg_lookup_ns = static_cast<double>(duration.count() * 1000) / (ITERATIONS * std::size(test_calls));
    ...
    std::cout << "  Avg per lookup: " << std::fixed << std::setprecision(2) 
