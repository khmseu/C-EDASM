...
bool test_actual_monitor_rom() {
    ...
    // Load actual ROM
    if (!bus.load_binary_from_file(0xF800, rom_path)) {
    ...
    if (reset_vec < 0xF800 || reset_vec > 0xFFFF) {
        std::cerr << "Reset vector $" << std::hex << reset_vec 
                  << " is outside ROM range" << std::endl;
        return false;
    }
