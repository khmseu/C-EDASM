...
struct RLDEntry {
    ...
    std::vector<uint8_t> to_bytes() const {
        return {flags, static_cast<uint8_t>(address & 0xFF), 
                static_cast<uint8_t>(address >> 8), symbol_num};
    }
...
struct ESDEntry {
    ...
    bool is_entry() const { return (flags & FLAG_ENTRY) != 0; }
    bool is_external() const { return (flags & FLAG_EXTERNAL) != 0; }
    bool is_relative() const { return (flags & FLAG_RELATIVE) != 0; }
    bool is_undefined() const { return (flags & FLAG_UNDEFINED) != 0; }
...
class RELFileBuilder {
  public:
    ...
    void add_esd_entry(const std::string& name, uint16_t address, uint8_t flags, uint8_t symbol_num = 0) {
        ESDEntry entry;
        entry.name = name;
        entry.address = address;
        entry.flags = flags;
    ...
    static bool parse(const std::vector<uint8_t>& data, 
        ...
        if (data.size() < 2 + code_len) return false;
    ...
    const std::vector<RLDEntry>& rld_entries() const { return rld_entries_; }
    const std::vector<ESDEntry>& esd_entries() const { return esd_entries_; }
