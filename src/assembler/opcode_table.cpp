// Opcode table implementation for EDASM assembler
//
// This file implements 6502/65C02 instruction encoding from EDASM.SRC/ASM/
...
void OpcodeTable::add(const std::string& mnem, AddressingMode mode, uint8_t code,
                      int bytes, int cycles, bool page_cross) {
    Opcode op;
    op.mnemonic = mnem;
    op.mode = mode;
    op.code = code;
...
AddressingMode AddressingModeDetector::detect(const std::string& operand, const std::string& mnemonic) {
